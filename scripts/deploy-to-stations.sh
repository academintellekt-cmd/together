#!/bin/bash
# Скрипт автоматического развертывания файлов на все станции через локальный сервер
# Использование: ./deploy-to-stations.sh [USERNAME] [PASSWORD] [STATION_PATH]

echo "🚀 Автоматическое развертывание файлов на станции..."
echo ""

# Параметры по умолчанию
USERNAME=${1:-pi}  # По умолчанию пользователь 'pi'
PASSWORD=${2:-""}   # Пароль (если пустой, будет запрошен)
STATION_PATH=${3:-"/home/pi/together"}  # Путь к проекту на станции
...
echo "   Путь на станции: $STATION_PATH"
echo ""
...
deploy_to_station() {
    ...
    # Создаем временный скрипт для развертывания на станции
...
echo "🌐 Исключаем локальные файлы (локальный режим не деплоится на станции)..."
...
if [ $SUCCESS_COUNT -gt 0 ]; then
    ...
    echo ""
    echo "1. Проверьте доступность станций:"
    ...
    echo ""
    echo "4. Если станции в другой подсети, измените IP адреса в скрипте"
    echo ""
    echo "5. Для подробной диагностики запустите:"
    echo "   DIAGNOSTIC_MODE=true ./scripts/deploy-to-stations.sh ${USERNAME}"
    echo ""
    exit 1
fi
