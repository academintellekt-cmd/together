#!/bin/bash
# Скрипт деплоя с автоматическим вводом пароля
SERVER_PASSWORD="t6LP6kJBE_9w663RR=Mc"
SERVER_IP=${1:-109.107.187.189}
USERNAME=${2:-root}

echo "🚀 Развертывание на $USERNAME@$SERVER_IP"
echo "📦 Создание архива..."
tar -czf quiz-deploy.tar.gz --exclude=node_modules --exclude=.git --exclude=*.backup --exclude=*.log --exclude=*.tar.gz --exclude=README.md --exclude=CHECKLIST.md --exclude=DEPLOY.md --exclude=DESIGN.md --exclude=RENDER_UPDATE.md --exclude=ИНСТРУКЦИЯ_ЗАПУСКА.md --exclude=УСТАНОВКА.md --exclude=public/fonts server.js package.json package-lock.json public/ scripts/start.sh nodemon.json server/ data/ tests/ docs/

echo "📤 Загрузка на сервер (пароль: $SERVER_PASSWORD)..."
